...
def test_generate_ugrep_config():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Verify content
        assert "Generated by fathom-mcp" in content
...
def test_write_ugrep_config():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert ugrep_path.parent != Path(tmpdir)  # Should be in temp dir
        assert "ugrep-fathom-mcp" in ugrep_path.name
...
def test_get_ugrep_config_path():
    """Test getting .ugrep config path."""
    with tempfile.TemporaryDirectory() as tmpdir:
        config = Config(knowledge=KnowledgeConfig(root=tmpdir))

        ugrep_path = config.get_ugrep_config_path()

        # Verify it's in temp directory
        assert ugrep_path.parent == Path(tempfile.gettempdir())
        assert ".ugrep-fathom-mcp" in ugrep_path.name

        # Verify it's unique per knowledge root
        config2 = Config(knowledge=KnowledgeConfig(root=tmpdir))
        ugrep_path2 = config2.get_ugrep_config_path()
        assert ugrep_path == ugrep_path2  # Same root, same path


def test_generate_ugrep_config_with_multiple_extensions():
    """Test .ugrep config generation with formats having multiple extensions."""
    with tempfile.TemporaryDirectory() as tmpdir:
        config = Config(knowledge=KnowledgeConfig(root=tmpdir))

        # Enable HTML format (has .html and .htm)
        config.formats["html"].enabled = True

        # Generate config
        content = config.generate_ugrep_config()

        # Verify both extensions are in the filter spec
        assert "html,htm:" in content or "htm,html:" in content
